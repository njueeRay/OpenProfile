...
| # | 功能 | 技术实现 | 状态 |
|---|------|---------|------|
| B-1 | **expressive-code 代码块** | `astro-expressive-code` + `github-dark-dimmed` 主题 + `borderRadius: 6px` + copy button | ⬜ 待开始 |
| B-2 | **Callout.astro 提示框** | 四类（tip/warning/info/note），终端风 icon（✦/⚠/ℹ/📝）+ uppercase 标签 + 对应色系 | ⬜ 待开始 |
| B-3 | **阅读时长显示** | 中文 500 字/分钟 + 英文 200 词/分钟，剥离 frontmatter/代码块/HTML；BlogCard + 文章页 header | ⬜ 待开始 |
| B-4 | **SeriesNav.astro 系列导航** | series + seriesOrder frontmatter + 系列内上/下篇导航；位置：author-card 后、相关文章前 | ⬜ 待开始 |
| B-5 | `astro check` 0 errors | 构建验证 | ⬜ 待开始 |
| B-6 | `npm run build` 构建成功 | 完整页数输出 | ⬜ 待开始 |
...
| # | 交付物 | 说明 | 状态 |
|---|--------|------|------|
| C-1 | **读者入口文章** | 「用 2 分钟了解 njueeray.github.io」，首屏就能看到 AI-native 核心主张 | ⬜ |
| C-2 | **Brand 72h 规则写入 DoD** | PM 在每个 Minor 版本提案时检查上一版本的外循环是否完成 | ⬜ |
| C-3 | **Discussion 月度读者反馈帖** | 邀请读者分享看法，作为外部数据输入 Sprint Board | ⬜ |
| C-4 | CHANGELOG [6.1.0] + git tag | 发版标准操作 | ⬜ |
