...
| D-3 | GitHub Discussion 开源宣布帖（v6.2.0 + 邀请关注）| 🔵 待发（gh auth 完成后执行）|

---

## v6.3.0 — Agent Persona Layer Phase 0（目标：2026-03-25）

**核心目标**：让 7 个 AI Agent 从「隐藏在 YAML 文件里的规范配置」变成「可被访客感知的鲜活角色」。

**触发条件**：v6.2.0 发布后自主规划，Sprint Board #7 추进完成

### DoD 清单

| # | 交付物 | 技术实现 | 状态 |
|---|--------|---------|------|
| E-1 | `src/content/config.ts` 扩展 authors schema | 新增字段：`color`（主题色 hex）/ `symbol`（emoji 图标）/ `joined`（加入日期）/ `tagline`（一句话简介）/ `skills`（能力标签数组）| ⬜ 待开始 |
| E-2 | 7 个 author YAML 填充（`.yml` 文件已存在）| brain/pm/dev/researcher/code-reviewer/profile-designer/brand 各自填充新字段 | ⬜ 待开始 |
| E-3 | `docs/strategy/agent-visual-spec.md` | 每个 Agent 的视觉定义：色彩含义 + 符号选择理由 + 角色素描（≤100 字）| ⬜ 待开始 |
| E-4 | `/agents` 路由页面（静态 SSG）| 7 张 Agent Card，布局参考 `/team` 页面；Card 包含：symbol / name / tagline / joined / skills-pills / 文章贡献数 | ⬜ 待开始 |
| E-5 | `astro check` + `npm run build` 通过 | 构建验证 | ⬜ 待开始 |

---

## v6.4.0 — Agent Activity Timeline（目标：2026-04-05）

**前置条件**：v6.3.0 完成

| # | 交付物 | 说明 |
|---|--------|------|
| F-1 | Agent Activity 数据脚本 | 扫描 `src/content/blog/` 按 `author` 聚合文章数 + 最近发布日 |
| F-2 | `/agents/[agent]` 详情页 | 个人简历卡 + Timeline（文章列表 + 时序轴）|
| F-3 | Brand Discussion：Agent 角色介绍帖 | 以每个 Agent 第一人称发一篇「我是谁，我做了什么」|

---

## 远期路线（v6.5+ → v7.x）

### v6.5.0 — `/agents/graph` 知识图谱

- 技术方案：react-force-graph（懒加载，岛屿架构）
- 数据：Agent 关系 JSON（协作边 + 决策权重）
- 前置：v6.4.0 完成
...
| v6.3.0 E1~E5 | #7 Agent Persona Layer Phase 0 |
