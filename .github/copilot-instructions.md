...
**v5.10.0 首页品牌化重设计（✅ 已发布 2026-03-10，tag: v5.10.0，commit: `1d2955d`）：**

> Hero 双栏叙事 + Projects API 动态化 + 视觉 Token 统一，首页升级为展示 AI-native 工作方式的完整门面。
- ✅ H-1 Hero 双栏叙事重设计：terminal-col (左 58%) + narrative-col (右，含 identity-badge / 中文标题 / 最新博文卡片 / quick-stats)
- ✅ H-2 Projects 区块 GitHub API 动态化：实时拉取 star / fork / 最近活跃，替换硬编码
- ✅ H-3 视觉语言全站统一：FeaturedCard / Projects 硬编码半径值 → 设计 token（radius-pill/sm/md）
- ✅ H-4 njueeRay-profile README 与站点双向联动：blog-posts.yml 已存在（RSS 驱动自动同步）+ Hero GitHub 按钮完成双向链接
